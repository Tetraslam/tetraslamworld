...
const typeOrder = ["anime", "manga", "book", "game", "music", "movie", "show", "other"];
...
function getMediaImages(item: MediaItem): string[] {
	...
	if (item._displayAsAlt && item.altImageOrder && item.altImageOrder.length === images.length) {
...
export default function MediaPage() {
	...
	const grouped = media?.reduce(
		...
		{} as Record<string, (typeof media extends (infer T)[] | undefined ? T & { _displayAsAlt?: boolean } : never)[]>,
	);
	...
	if (grouped) {
		for (const type of Object.keys(grouped)) {
			grouped[type]?.sort((a, b) => {
				const orderA = a._displayAsAlt ? (a.altOrder ?? a.order ?? 0) : (a.order ?? 0);
				const orderB = b._displayAsAlt ? (b.altOrder ?? b.order ?? 0) : (b.order ?? 0);
	...
	return (
		...
		>
				...
				{images.length > 0 ? (
					<>
						{images.map((url, index) => (
							// biome-ignore lint: dynamic image slideshow
							<img
								key={url}
								src={url}
			...
			<div className="p-2 border-t border-border/50 flex items-center justify-between gap-1">
				...
				{item.content ? (
					<svg className="md:hidden w-4 h-4 text-rose/70 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<div className="relative bg-surface border border-border rounded-lg max-w-lg w-full max-h-[80vh] overflow-auto animate-fade-in">
				...
				{images.length > 0 && (
					<div className="relative">
						<div className="relative overflow-hidden rounded-t-lg bg-background/50 flex items-center justify-center h-[400px]">
							{images.map((url, index) => (
								// biome-ignore lint: dynamic image gallery
								<img
									key={url}
									src={url}
									alt={`${item.title} ${index + 1}`}
